# functions for downloading and handling MERRA-2 data
...
getMERRADataBox<-function(lon1,lat1,lon2,lat2,period,params,user,pass,runParallel=TRUE){
  ...
  for(i in 1:length(period)){
    ...
    url<-paste("http://goldsmr4.gesdisc.eosdis.nasa.gov/daac-bin/OTF/HTTP_services.cgi?FILENAME=%2Fdata%2FMERRA2%2FM2I1NXASM.5.12.4%2F",
               ...
               ".SUB.nc4&SHORTNAME=M2I1NXASM&SERVICE=L34RS_MERRA2&VERSION=1.02&LAYERS=&VARIABLES=",variables,
               sep="")
...
convertMerraFeather<- function(date_seq,lon1,lat1,lon2,lat2,pname,pname1) {
  ...
  dates <- format(unlist(date_seq),format="%Y%m%d")
  #dates <- format(date_seq2,"%Y%m%d")
  ...
  df<-bind_rows(res) #Matriz: Linhas (diasx24h) e colunas (combinacao de lat x log) 
  
  ###write merra points for whole period
  dir<-paste(dirmerrabase,"/feather",sep = "")
  dir.create(file.path(dir), showWarnings = TRUE)
  dir<-paste(paste0(dirmerrabase,"/feather/LonLat"),pname,lon1,lat1,lon2,lat2,
             format(date_seq[1],"%Y%m%d"),
             format(date_seq[length(unlist(date_seq))],"%Y%m%d"),sep="_")
  dir.create(file.path(dir), showWarnings = TRUE)
  ...
  }#Escreve um arquivo para cada combinacao de lat com log (dentro de cada arquivo tem dias x 24h)

  write_feather(lonlat,paste(dir,"/lonlat.feather",sep="")) # Escreve arquivo mostrando todas combinacoes de lat com log
}
...
} # Pega as 24 velocidades para cada combinação lat com Log para cada dia




##### merges merra variables from five to one file
mergeMERRAvars <- function(date_seq,pnames,long,lat,lon1,lat1,lon2,lat2){
  dir<-paste(paste0(dirmerrabase,"/feather/LonLat"),pnames,lon1,lat1,lon2,lat2,
             format(date_seq[1],"%Y%m%d"),
...
##### merges merra variables and saves new dataset
...
##### reads merra data for one point from disk
getMerraPoint <- function(long,lat){
  x <- read_feather(paste(dirmerra,"/",long,"_",lat,".feather",sep=""))
  time <- read_feather(paste(dirmerra,"/MerraDate.feather",sep=""))
  return(cbind(time,x))
}
